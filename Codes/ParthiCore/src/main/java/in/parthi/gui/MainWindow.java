...
public class MainWindow {
	...
	private void setAddProductForm() {
		...
		JLabel lblProductInvoice = new JLabel("Invoice");
		lblProductInvoice.setHorizontalAlignment(SwingConstants.RIGHT);
		lblProductInvoice.setFont(new Font("Tahoma", Font.BOLD, 14));
		lblProductInvoice.setBounds(179, 83, 96, 24);
		panelAddProduct.add(lblProductInvoice);

		textProductInvoice = new JTextField();
		textProductInvoice.setToolTipText("Give Product ID in format:  \"SA-\"");
		textProductInvoice.setFont(new Font("Dialog", Font.PLAIN, 12));
		textProductInvoice.setColumns(10);
		textProductInvoice.setBounds(286, 83, 147, 24);
		panelAddProduct.add(textProductInvoice);
		...
		lblVendor.setBounds(179, 431, 96, 24);
		panelAddProduct.add(lblVendor);

		textVendor = new JTextField();
		textVendor.setToolTipText("Give Product ID in format:  \"SA-\"");
		textVendor.setFont(new Font("Dialog", Font.PLAIN, 12));
		textVendor.setColumns(10);
		textVendor.setBounds(286, 431, 147, 24);
		panelAddProduct.add(textVendor);
	...
	private void addProduct() {
		...
		try {
			...
			if (id == null || textProductInvoice.getText().trim().length() < 1)
				throw new Exception("Product Invoice cannot be blank");
			...
			// Adding the corresponding transaction
			AddProductTransaction addProductTransaction = new AddProductTransaction();
			addProductTransaction.setInvoice(textProductInvoice.getText());
			addProductTransaction.setAmount(product.getCostPrice());
			addProductTransaction.setProductId(product.getId());
			addProductTransaction.setVendor(textVendor.getText());
			addProductTransaction.setPaymentMode(comboBoxAddPrdpayMode.getSelectedItem().toString());
			addProductTransaction.setTransactionDate(product.getStockInDate());

			response += " \n" + transactionService.addProductTransaction(addProductTransaction);
